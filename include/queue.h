...
#define MAX_CQ_ENTRIES_2M (2 * 1024 * 1024 / 16)
    ...
    inline QueuePair(const nvm_ctrl_t *ctrl, const struct nvm_ns_info ns, const struct nvm_ctrl_info info, nvm_aq_ref &aq_ref, const uint16_t qp_id, const uint64_t queueDepth)
    {
        ...
        uint64_t sq_size = (cqr) ? ((MAX_SQ_ENTRIES_2M <= ((((volatile uint16_t *)ctrl->mm_ptr)[0] + 1))) ? MAX_SQ_ENTRIES_2M : ((((volatile uint16_t *)ctrl->mm_ptr)[0] + 1))) : ((((volatile uint16_t *)ctrl->mm_ptr)[0] + 1));
        uint64_t cq_size = (cqr) ? ((MAX_CQ_ENTRIES_2M <= ((((volatile uint16_t *)ctrl->mm_ptr)[0] + 1))) ? MAX_CQ_ENTRIES_2M : ((((volatile uint16_t *)ctrl->mm_ptr)[0] + 1))) : ((((volatile uint16_t *)ctrl->mm_ptr)[0] + 1));
