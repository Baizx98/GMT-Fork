...
__forceinline__ __device__ unsigned long long int get_elapsed_seconds()
{
    return (clock64() - clock_begin) / CLOCK_FREQ_GPU;
}

__global__ void get_clock_begin()
{
    clock_begin = clock64();
}

inline __host__ host_ring_buffer_t *init_ring_buf_for_dev(uint32_t q_size, host_ring_buffer_t *host_buf)
{
...
__forceinline__ __device__ void enqueue_mem_samples(host_ring_buffer_t *buf, uint64_t key, uint64_t virt_timestamp_diff, callback_t callback_func, uint64_t *ring_count)
{
    uint32_t ns = 8;
...
__forceinline__ __device__ void enqueue_mem_samples_blocked(host_ring_buffer_t *buf, uint64_t key, uint64_t virt_timestamp_diff, callback_t callback_func, uint64_t *ring_count)
{
    uint32_t ns = 8;
    ulonglong2 sample = make_ulonglong2(virt_timestamp_diff, key);
    uint64_t ticket = buf->in_ticket.val.fetch_add(1, simt::memory_order_relaxed);
    ...
    if ((_count & (buf->q_size - 1)) == buf->q_size - 1)
    {
        ...
        (*ring_count)++;
        while (*(buf->ready))
            ; // wait until ready flag becomes false
...
inline __host__ void dequeue_mem_samples()
{
}
