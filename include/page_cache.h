...
struct tlb
{
    ...
    */
    ...
    __forceinline__
        __device__ T *
        acquire(const size_t i, const size_t gid, size_t &start, size_t &end, range_d_t<T> *range, const size_t page_)
    {
...
struct bam_ptr_tlb
{
    ...
    __forceinline__
        __device__ ~bam_ptr_tlb() { fini(); }

    __forceinline__
        __host__ __device__ void
        init(array_d_t<T> *a, tlb<T, n, _scope, loc> *t)
    {
        array = a;
        tlb_ = t;
    }
...
#define all_pages_evicted() (dirty_pages_evicted->load() + clean_pages_evicted->load())
    ...
    __forceinline__
        __device__
            uint32_t
            find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t *bid = NULL);
    ...
    __forceinline__
        __device__
            ...
            uint32_t
            find_slot_prefetching(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t *bid = NULL);

    __forceinline__
        __device__
            uint64_t
            get_cache_page_addr(const uint32_t page) const;
...
// CHIA-HAO
// #if USE_HOST_CACHE
__global__ void __flush_to_hc(page_cache_d_t *pc)
{
    uint64_t page = threadIdx.x + blockIdx.x * blockDim.x;
...
struct range_d_t
{
    ...
    __forceinline__
        __device__
            uint64_t
            find_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl, const uint32_t queue, bool &nv_nb);
    /////////////
    __forceinline__
        __device__
            uint64_t
            find_page_without_data_transfer(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue, uint32_t &scheme);
    __forceinline__
        __device__ void
        get_page_from_ssd(const size_t pg, const bool write, const uint32_t ctrl, const uint32_t queue, uint64_t page_trans);
    __forceinline__
        __device__ void
        write_page_to_ssd(const size_t pg, const bool write, const uint32_t ctrl, const uint32_t queue, uint64_t page_trans);
    __forceinline__
        __device__ void
        update_page_state(const uint64_t pg, uint32_t new_s);
    __forceinline__
        __device__ void
        disable_page_state(const uint64_t pg, uint32_t state);
    __forceinline__
        __device__ void
    ...
    __forceinline__
        __device__ void
        mark_page_dirty(const size_t index);
};
...
__forceinline__
    __device__ void
    range_d_t<T>::mark_page_dirty(const size_t index)
{
...
__forceinline__
    __device__
        uint64_t
        range_d_t<T>::acquire_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue)
{
    uint64_t index = pg;
...
__forceinline__
    __device__
        uint64_t
        range_d_t<T>::find_page(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue, bool &nv_nb_and_cached_by_hc)
{
    uint64_t index = pg;
    access_cnt.fetch_add(count, simt::memory_order_relaxed);
    bool fail = true;
...
__forceinline__
    __device__
        uint64_t
        range_d_t<T>::find_page_without_data_transfer(const size_t pg, const uint32_t count, const bool write, const uint32_t ctrl_, const uint32_t queue, uint32_t &scheme)
{
    uint64_t index = pg;
    access_cnt.fetch_add(count, simt::memory_order_relaxed);
    bool fail = true;
...
__forceinline__
    __device__ void
    range_d_t<T>::get_page_from_ssd(const size_t pg, const bool write, const uint32_t ctrl, const uint32_t queue, uint64_t page_trans)
{
    uint64_t index = pg;
    uint64_t b_page = get_backing_page(index);
    Controller *c = cache.d_ctrls[ctrl];
...
__forceinline__
    __device__ void
    range_d_t<T>::write_page_to_ssd(const size_t pg, const bool write, const uint32_t ctrl, const uint32_t queue, uint64_t page_trans)
{
    uint64_t index = pg;
    uint64_t b_page = get_backing_page(index);
    Controller *c = cache.d_ctrls[ctrl];
...
__forceinline__
    __device__ void
    range_d_t<T>::update_page_state(const uint64_t pg, uint32_t new_s)
{
...
__forceinline__
    __device__ void
    range_d_t<T>::disable_page_state(const uint64_t pg, uint32_t state)
{
...
struct array_d_t
{
    ...
    __forceinline__
        __device__ void
        get_page_gid(const uint64_t i, range_d_t<T> *&r_, size_t &pg, size_t &gid) const
    {
    ...
    __forceinline__
        __device__ int
        zero_copy(const int64_t r, const uint64_t page, uint32_t eq_mask, const uint32_t lane, const int master, uint64_t base_master, bool dirty, uint32_t queue) const
    {
        ///*
        // printf("zero copy\n");
        auto r_ = d_ranges + r;
    ...
    __forceinline__
        __device__ int
        evict_and_zero_copy(const int64_t r, const uint64_t page_out, const uint64_t page_in, uint32_t eq_mask, const uint32_t lane, const int master, uint64_t base_master, bool dirty, uint32_t queue) const
    {
        ///*
        // printf("zero copy\n");
        auto r_ = d_ranges + r;
    ...
    __forceinline__
        __device__ int
        zero_copy_test(const int64_t r, const uint64_t page, uint32_t eq_mask, const uint32_t lane, const int master, uint64_t base_master, bool dirty, uint32_t queue) const
    {
        ///*
        auto r_ = d_ranges + r;
        int ret = HOST_CACHE_SUCCEEDED;
    ...
    __forceinline__
        __device__ void
        coalesce_page_2(const uint32_t lane, const uint32_t mask, const int64_t r, const uint64_t page, const uint64_t gaddr, const bool write,
...
// #endif
    ...
    __forceinline__
        __device__ void *
        acquire_page_(const size_t i, data_page_t *&page_, size_t &start, size_t &end, range_d_t<T> *r_, const size_t page) const
    {
        // uint32_t lane = lane_id();
    ...
    __forceinline__
        __device__ void *
        acquire_page(const size_t i, data_page_t *&page_, size_t &start, size_t &end, int64_t &r) const
    {
        uint32_t lane = lane_id();
        r = find_range(i);
        auto r_ = d_ranges + r;
    ...
    __forceinline__
        __device__ void
        release_page(data_page_t *page_, const int64_t r, const size_t i) const
    {
        uint32_t lane = lane_id();
        auto r_ = d_ranges + r;
...
__forceinline__
    __device__
        uint64_t
        page_cache_d_t::get_cache_page_addr(const uint32_t page) const
{
...
__forceinline__
    __device__ void
    page_cache_d_t::evict_all_to_hc()
{
...
__forceinline__
    __device__
        uint32_t
        page_cache_d_t::find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t *bid)
{
    bool fail = true;
    uint64_t count = 0;
...
// 072223
                                        ...
                                        if (!reuse_place_decision &&
                                            // if (
                                            tier_to_evict == Tier3_SSD && all_pages_evicted() >= (total_pages_num >> 1) && ranges_reuse_hist[previous_range][previous_address].evicted_before > 0)
                                        {
...
// get "real" reuse distance if the flag is enabled
                                ...
                                if (!reuse_place_decision &&
                                    // if (
                                    tier_to_evict == Tier3_SSD && all_pages_evicted() >= (total_pages_num >> 1) && dirty_pages_evicted->load() < special_handling_threshold() && ranges_reuse_hist[previous_range][previous_address].evicted_before > 0)
                                {
                                    if (!idle_slots_too_few(0))
                                        tier_to_evict = Tier2_CPU;
...
__forceinline__
    __device__
        uint32_t
        page_cache_d_t::find_slot_prefetching(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t *bid)
{
    bool fail = true;
    uint64_t count = 0;
    uint32_t global_address = (uint32_t)((address << n_ranges_bits) | range_id); // not elegant. but hack
