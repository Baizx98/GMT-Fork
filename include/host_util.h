...
template <typename T>
inline __host__ void __nanosleep(T ns)
{
...
template <typename T>
inline __host__ int __popc(T v)
{
    if (sizeof(T) == 4)
...
template <typename T>
inline __host__ void __syncwarp(T mask)
{
    (void)mask;
    return;
}
...
template <typename T>
inline __host__ unsigned int __match_any_sync(unsigned mask, T var)
{
    (void)mask;
    (void)var;
    return 1;
