...
#define WAIT_ON_MEM(mem, val)               \
    while (read_without_cache(&mem) != val) \
        ;
#define WAIT_ON_MEM_NE(mem, val)            \
    while (read_without_cache(&mem) == val) \
        ;
...
{
public:
    MemSampleCollector(LinearRegInfo *_info, uint32_t *_info_idx, host_ring_buffer_t *_ring_buf_h, host_ring_buffer_t *_ring_buf_d) : linear_reg_info_host(_info), linear_reg_info_idx_host(_info_idx), ring_buf_h(_ring_buf_h), ring_buf_d(_ring_buf_d)
    {
        this->size = 16384;
    ...
    uint32_t calculate_reuse_dist_for_samples(uint64_t *samples)
    {
        ...
        for (uint32_t i = 0; i < ring_buf_h->q_size; i++)
        {
            ...
            if (estimated_reuse_dist != NO_REUSE)
            {
                ...
                if (samples[2 * i] / samples[2 * i + 1] > 100000)
                    continue;
#endif

#if BYPASS_INVALID_SAMPLES
                if (samples[2 * i + 1] > samples[2 * i])
                    continue;

#if PRINT_TRAINING_PAIRS
                printf("%lu,%lu\n", samples[2 * i], samples[2 * i + 1]);
#endif
                if ((samples[2 * i] & 0x80000000) != 0)
                    continue;
...
__device__
    uint32_t
    getTierByReuseDistance(uint64_t reuse_dist)
{
    uint32_t ret = 0;
...
__forceinline__ __device__ int writeDataToHost(int _fd, size_t _size, off_t _offset, void *gpu_addr)
{
...
__forceinline__ __device__ int accessHostCache(void *gpu_addr, uint64_t key, uint32_t req_type, int *bid, bool dirty, bool *is_cached_page_dirty = NULL)
{
...
__forceinline__ __device__ int sendReqToHostCache(uint64_t key, int bid, uint32_t req_type, bool *is_cached_page_dirty = NULL)
{
    uint8_t ns = 8;
    LocalEntry local;
    local.type = req_type;
...
__forceinline__ __device__ int pushMemSampleToHost(uint64_t key, int req = GPU_RW_SAMPLE_MEM)
{
    uint8_t ns = 8;
    LocalEntry local;
    local.type = req;
...
__forceinline__ __device__ bool getPredictionResult(uint64_t actual_reuse_dist, uint32_t predicted_tier)
{
...
__forceinline__ __device__ int accessHostCacheAsync(uint64_t tag, void *gpu_addr, int __req_type)
{
    // int req_type                        = __req_type;
    int rw_queue_index = -1;
    // void* buffer_ptr                    = NULL;
...
__forceinline__ __device__ int copyFromHostCache(void *data_in, void *data_out, size_t bytes, uint32_t eq_mask)
{
...
__forceinline__ __device__ int copyFromHostCacheVec(void *data_in, void *data_out, size_t bytes, uint32_t eq_mask)
{
...
__forceinline__ __device__ void loggingPageAction(uint64_t arrayIdx, uint64_t pageIdx, uint32_t actionType)
{
    uint8_t ns = 8;
...
__forceinline__ __device__ void loggingRtInfo(uint64_t thread_count)
{
    uint8_t ns = 8;
...
__forceinline__ __device__ void sampleMemAccess(uint64_t arrayIdx, uint64_t pageIdx, uint64_t virt_timestamp_dist)
{
    uint8_t ns = 8;
...
__device__ void notifyHostForSamples(uint64_t tail)
{
    uint8_t ns = 8;
    uint32_t req_type = GPU_RW_SAMPLE_MEM;
    int32_t rw_queue_index = 0xFFFFFFFF;
...
__device__ void testCallback(uint64_t key)
{
...
__device__
    uint64_t
    estimateReuseDistance(uint64_t virt_timestamp_dist)
{
    uint64_t ret;
...
__device__
    uint32_t
    which_tier_to_evict(PageReuseTable *prt, uint64_t curr_virt_timestamp, bool &reuse_place_decision, bool dirty = false)
{
...
//__forceinline__ __device__
__device__ void update_timestamp_upon_eviction(PageReuseTable *prt, uint64_t curr_virt_timestamp)
{
...
//__forceinline__ __device__
__device__ void update_timestamp_upon_re_reference(PageReuseTable *prt, uint64_t curr_virt_timestamp)
{
...
__forceinline__ __device__ void update_tier_bins(uint32_t tier)
{
...
// Lastly, do data transfer
    ...
    else if (type == GPU_RW_PIN_PAGE)
    {
        ...
        if (host_cache_state[_bid].state.load(simt::memory_order_acquire) != HOST_CACHE_ENTRY_VALID || host_cache_state[_bid].tag != key)
        {
...
void HostCache::mainLoop()
{
    ...
    do
    {
        ...
        if (this->host_rw_queue->entries[queue_index].status == GPU_RW_WAIT_FLUSH)
        {
            ...
            if (err != cudaSuccess)
                fprintf(stderr, "Flush GPU Direct RDMA wirtes failed.\n");
            this->host_rw_queue->entries[queue_index].status = GPU_RW_READY;
            __sync_synchronize();
        }
        if (this->host_rw_queue->entries[queue_index].status == GPU_RW_PENDING)
