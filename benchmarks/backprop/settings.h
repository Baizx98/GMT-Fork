...
void Settings::parseArguments(int argc, char **argv)
{
    OptionMap parsers = {
        ...
        {'c', OptionPtr(new Range(cols, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "cols", "cols", "0"))},
        {'r', OptionPtr(new Range(rows, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "rows", "rows", "0"))},
        {'L', OptionPtr(new Range(layer_size, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "layer_size", "layer_size", "16777216"))},
        {'h', OptionPtr(new Range(pyramid_height, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "Pyramid Height", "pyramid-height", "0"))},
        {'i', OptionPtr(new Range(iter, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "iterations", "iterations", "10"))},
    };
