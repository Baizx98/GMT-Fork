...
__global__ void reduce_sum(int hid, int num_blocks, TYPE *partial_sum, TYPE *out)
{
...
__global__ void reduce_sum(int64_t hid, int64_t num_blocks, array_d_t<TYPE> *partial_sum, array_d_t<TYPE> *out)
{
    // extern __shared__ TYPE sdata[];
    __shared__ TYPE sdata[512];
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void activate_kernel(array_d_t<TYPE> *hidden_units, array_d_t<TYPE> *buffer, array_d_t<TYPE> *input_weights, int64_t hid)
{
...
extern "C" TYPE *alloc_1d_dbl(int n)
//    int n;
{
    TYPE *_new;
...
void bpnn_randomize_weights(TYPE **w, int m, int n)
//    TYPE **w;
//    int m, n;
{
    int i, j;
...
void bpnn_randomize_row(TYPE *w, int m)
//    TYPE *w;
//    int m;
{
    int i;
...
void bpnn_zero_weights(TYPE **w, int m, int n)
//    TYPE **w;
//    int m, n;
{
    int i, j;
...
__global__ void bpnn_layerforward_kernel(array_d_t<TYPE> *l1, array_d_t<TYPE> *l2, array_d_t<TYPE> *conn, int n1, int n2)
{
    TYPE sum;
    int j, k;
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void bpnn_output_error_kernel(array_d_t<TYPE> *delta, array_d_t<TYPE> *target, array_d_t<TYPE> *output, int nj)
{
    int j;
    TYPE o, t, errsum;
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void bpnn_hidden_error_kernel(array_d_t<TYPE> *delta_h,
                                         int nh,
...
__global__ void bpnn_adjust_weights_kernel(array_d_t<TYPE> *delta, int ndelta /*out*/, array_d_t<TYPE> *ly, int nly /*hid*/, array_d_t<TYPE> *w, array_d_t<TYPE> *oldw)
// TYPE *delta, *ly, **w, **oldw;
{
    TYPE new_dw;
    int k, j;
...
extern "C" void bpnn_feedforward(BPNN *net)
// BPNN *net;
{
    int in, hid, out;
...
extern "C" void bpnn_train(BPNN *net, TYPE *eo, TYPE *eh)
// BPNN *net;
// TYPE *eo, *eh;
{
    int in, hid, out;
...
extern "C" void bpnn_save(BPNN *net, char *filename)
// BPNN *net;
// char *filename;
{
...
extern "C" BPNN *bpnn_read(char *filename)
// char *filename;
{
    char *mem;
    BPNN *_new;
...
extern "C" void bpnn_train_cuda(BPNN *net, TYPE *eo, TYPE *eh)
{
    int in, hid, out;
    TYPE out_err, hid_err;
    ...
    //*/
        ...
        if (settings.memalloc == BAFS_DIRECT)
        {
            ...
            if (error != cudaSuccess)
            {
                ...
                printf("bpnn layerforward kernel \n");
            cuda_err_chk(cudaStreamSynchronize(stream_mngr->kernel_stream));
            ///*
            ...
            if (error != cudaSuccess)
            {
                ...
                printf("bpnn output error kernel \n");
            cuda_err_chk(cudaStreamSynchronize(stream_mngr->kernel_stream));
            ...
            if (error != cudaSuccess)
            {
                ...
                printf("bpnn hidden error kernel \n");
            cuda_err_chk(cudaStreamSynchronize(stream_mngr->kernel_stream));
            //*/
            ...
            if (error != cudaSuccess)
            {
                ...
                printf("bpnn adjust weights kernel \n");
            //*/
            cuda_err_chk(cudaStreamSynchronize(stream_mngr->kernel_stream));
#else
        ...
        if (settings.memalloc == BAFS_DIRECT)
        {
            ...
            if (error != cudaSuccess)
            {
                ...
                printf("bpnn adjust weights cuda kernel \n");
            cuda_err_chk(cudaStreamSynchronize(stream_mngr->kernel_stream));
...
void backprop_face()
{
    ...
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_hidden_partial_sum_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_buffer_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_buffer_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_output_units_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_hidden_weights_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_output_delta_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_target_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_hidden_delta_pages;
    hc->registerRangesLBA(acc_pages * settings.pageSize / 512);
    acc_pages += net->n_hidden_prev_weights_pages;
#endif
